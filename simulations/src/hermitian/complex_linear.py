...
class ComplexLinear(nn.Module):
    ...
    Les biais réel/imaginaire sont des paramètres dédiés (pas ceux d'un
    `nn.Linear` interne) : réutiliser le biais de deux `nn.Linear` dans les
    deux équations ferait fuiter B_real dans Y_imag (et B_imag dans
    Y_real) — un bug qui ne se voit qu'en comparant à une référence
    externe à biais non nul (cf. test I-01, portage de poids HuggingFace).
    """
    ...
    def forward(
        ...
        out_real = self.fc_real(x_real) - self.fc_imag(x_imag) + self.bias_real
        out_imag = self.fc_real(x_imag) + self.fc_imag(x_real) + self.bias_imag
