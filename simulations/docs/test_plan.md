...
| I-05 | I-04 vert | Run complet : 10 tests (`Q_global` × 5 `nN`, `Q_i` agrégé × 5 `nN`), `M=300` par corrélation (`dt=1.0`, motifs `src/superposition/experiment.py::generate_patterns`, seeds archivées dans `docs/results/i05_run_2026-09-13.json`) | Violation ≥5σ (par test), correction de Bonferroni sur la famille de 10 | **PARTIEL** (2026-09-13) — 2/10 tests franchissent 5σ (`nN=5` local σ=5,67 ; `nN=20` local σ=6,88) ; `Q_global` ne franchit jamais 5σ (max σ=3,38 à `nN=5`) ; `nN=3` local ne montre même pas de violation nominale (σ=−1,71) ; aucune tendance monotone claire avec `nN`. **Non citable dans `Simulations_API.md`** en l'état (résultat mixte, pas stable) — cf. `docs/DevPlan.md` pour l'analyse et les limites méthodologiques identifiées. |
